		...
		wrapper_clicked: function (e) {
			...
			$('body').unbind().on('click', '.day_id-' + $(this).attr('id'), function () {
				if ($(this).html() != "&nbsp;") {
					...
					// //yc：用于获取当前点击的日期是多少
					// selectedDate = formatted_date.replace(/\s*/g,"");;
					// console.log(selectedDate);

					// //yc：接下来用于设定早餐，午餐，晚餐，加餐的食物有哪些，并且显示在input框里面
					// var tmpArr = new Array();
					// tmpArr.push("麻辣香锅")
					// tmpArr.push("豆浆")

					// var selectElement = $("#breakfast").parent().parent().parent()

					// for(var i=0;i<tmpArr.length;i++){
					// 	var a = $('<a />').addClass('notShown shown').html('<em>' + tmpArr[0] + '</em><i></i>');
					// 	a.appendTo(selectElement.children('div'));
					...
					//yc：用于发送ajax请求
					$.ajax({
						url: hostURL + "/dailyfood/" + selectedDate,
						type: "get",
						success: function (data) {
							...
							for (var i = 0; i < data.breakfast.length; i++) {
								...
								//yc：接下来用于设定早餐，午餐，晚餐，加餐的食物有哪些，并且显示在input框里面
								var selectElement = $("#breakfast").parent().parent().parent()
								var a = $('<a />').addClass('notShown').html('<em>' + data.breakfast[i] + '</em><i></i>').addClass("shown").appendTo(selectElement.children('div'))
								// a.addClass('shown');
								selectElement.children('div').children('span').addClass('hide');
								selectElement.find('option:contains(' + data.breakfast[i] + ')').prop('selected', true);
							}

							for (var i = 0; i < data.lunch.length; i++) {
								console.log(data.lunch[i]);
								//yc：接下来用于设定早餐，午餐，晚餐，加餐的食物有哪些，并且显示在input框里面
								var selectElement = $("#lunch").parent().parent().parent()
								var a = $('<a />').addClass('notShown').html('<em>' + data.lunch[i] + '</em><i></i>').addClass("shown").appendTo(selectElement.children('div'))
								selectElement.children('div').children('span').addClass('hide');
								selectElement.find('option:contains(' + data.lunch[i] + ')').prop('selected', true);
							}

							for (var i = 0; i < data.dinner.length; i++) {
								console.log(data.dinner[i]);
								//yc：接下来用于设定早餐，午餐，晚餐，加餐的食物有哪些，并且显示在input框里面
								var selectElement = $("#dinner").parent().parent().parent()
								var a = $('<a />').addClass('notShown').html('<em>' + data.dinner[i] + '</em><i></i>').addClass("shown").appendTo(selectElement.children('div'))
								selectElement.children('div').children('span').addClass('hide');
								selectElement.find('option:contains(' + data.dinner[i] + ')').prop('selected', true);
							}

							for (var i = 0; i < data.others.length; i++) {
								console.log(data.others[i]);
								//yc：接下来用于设定早餐，午餐，晚餐，加餐的食物有哪些，并且显示在input框里面
								var selectElement = $("#extraFood").parent().parent().parent()
								var a = $('<a />').addClass('notShown').html('<em>' + data.others[i] + '</em><i></i>').addClass("shown").appendTo(selectElement.children('div'))
								selectElement.children('div').children('span').addClass('hide');
								selectElement.find('option:contains(' + data.others[i] + ')').prop('selected', true);
							}
